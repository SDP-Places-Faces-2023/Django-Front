...
export class UnrecognizedComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<UnrecognizedComponent>,
  ...
  deleteFolder(date) {
    let url = '/model_api_connection/delete_unrecognized_images/?date=' + date
    ...
    this.httpService.getData(url, {}).subscribe((res) => {
      if(res.success) {
        this.openSnackBar("Folder deleted", 'DELETE');
        this.getFolders();
      } else {
        this.openSnackBar(`${res.response.error}`, 'ERROR');
      }
      this.loading = false
    });
  }
  ...
  getFolders() {
    let url = '/model_api_connection/get_unrecognized_folders/'
  ...
  getImages(date) {
    let url = '/model_api_connection/get_unrecognized_faces/?date=' + date
